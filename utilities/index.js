...
Util.buildInventoryDetailsGrid = async function (data) {
  ...
  if (vehicle) {
    ...
    grid +=
      '<div class="details-images"> <img src="' +
      vehicle.inv_image +
      '" alt="Image of ' +
      vehicle.inv_make +
      " " +
...
Util.selectAllClassification = async function (selectedClassificationId) {
  ...
  let select =
    '<label for="classification_id">Select Classification: </label><br>';
...
Util.checkAccountType = (isLoggedIn, accountType) => {
  ...
  if (isLoggedIn && (accountType === "Admin" || accountType === "Employee")) {
    ...
    managementGrid +=
      '<a id="inv-management-button" href="../../inv/" title="Inventory Management View "><h3>Manage Inventory</h3></a>';
  } else {
    managementGrid = "";
  }
